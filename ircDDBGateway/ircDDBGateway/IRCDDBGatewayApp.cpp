...
void CIRCDDBGatewayApp::createThread()
{
	...
	bool xlxEnabled;
	m_config->getXLX(xlxEnabled);
	wxLogInfo(wxT("XLX enabled: %d"), int(xlxEnabled));
	...
	thread->setXLX(xlxEnabled, xlxEnabled ? CXLXHostsFileDownloader::Download() : wxString(wxEmptyString));
